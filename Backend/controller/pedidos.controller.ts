...
export async function getAuthUserPedidos(req: Request, res: Response) {
    ...
    const query = `
        SELECT p.id as pedidoId, p.fecha, p.usuario, p.estado, pr.nombre as productoNombre, pp.cantidad, pr.precio, pr.imagen 
        FROM pedido p 
...
export async function getAll(req: Request, res: Response) {
    const query = `
        SELECT p.id as pedidoId, u.nombre as usuario, p.fecha, p.estado, pr.nombre as productoNombre, pp.cantidad, pr.precio, pr.imagen 
        FROM pedido p 
        JOIN productos_pedidos pp ON p.id = pp.pedido
        JOIN producto pr ON pr.id = pp.producto
...
function procesarPedidos(results: RowDataPacket[]) {
        ...
        for (const row of results) {
            ...
            if (row.productoNombre) {
                console.log('Agregando producto al pedido', row.pedidoId, row.productoNombre);
...
export async function actualizarEstado(req: Request<{ id?: number }, {}, {estado: string}>, res: Response) {
    ...
    `;
    console.log(req.params.id, req.body.estado);
    db.execute(query, [req.body.estado, req.params.id!], (err, results) => {
        if (err) {
            console.log('Error actualizando estado del pedido', err);
            return res.status(500).json({
            mensaje: "Error actualizando estado: " + err.message
        });}
        return res.status(200).json({ mensaje: 'Estado actualizado con éxito' });
    });
}